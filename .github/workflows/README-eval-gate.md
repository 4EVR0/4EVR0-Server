...
Hard gate는 `eval/hard_checks.py`가 빈 응답, 한자, 근거 없는 제품 제약,
제품-성분 연결, 제품 목적, 브랜드 중복, 금칙 표현을 코드로 검사한다. 런타임 가드가
안전한 응답으로 고친 경우는 통과하고, 최종 결과에 위반이 남은 경우만 실패한다.
검색 공백은 고민이 있고 미검증 제품 제약이 없는 케이스만 게이트에 센다. 고민 없음이나
무향·저자극처럼 현재 데이터로 입증할 수 없는 요청의 제품 0건은 안전한 정상 결과다.
