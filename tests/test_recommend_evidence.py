...
class EvidenceLabelTest(unittest.TestCase):
    def test_reviewed_retinol_study_is_limited_to_matching_face_wrinkle_template(self):
        ...
        self.assertIn("얼굴 주름 개선 연구를 입가 주름 고민에 참고할 수 있지만", response)
        self.assertIn("입가에도 같은 결과가 나타나는지는 확인되지 않았습니다", response)
        self.assertIn("제품 데이터에서 연구 대상 성분인 레티놀이 확인되어 후보로 골랐습니다", response)
        self.assertIn("이 제품에 같은 효과가 있는지는 확인되지 않았습니다", response)
        ...
        self.assertNotIn("눈가에도 같은 결과가 나타나는지는 확인되지 않았습니다", eye_response)
        self.assertIn("이 제품에 같은 효과가 있는지는 확인되지 않았습니다", eye_response)
