...
- 주 판정 축은 `concern_fit`, `grounding`, `korean_quality`다. 이 세 축의 케이스별 평균으로
  judge의 방향성·편향을 판단한다.
- `conciseness`, `format_adherence`는 스타일 선호와 해석 차이가 큰 보조 진단 축이다.
  기준이 불명확한 상태에서 이 두 축을 핵심 품질 종합점수에 같은 가중치로 섞지 않는다.
- 다음 라벨링부터는 보조 축의 기준 예시를 먼저 합의하거나, 확신도/판단 불가 표시를 함께
  수집한다. 애매하다는 이유로 같은 중간 점수에 몰아넣으면 상관 분석이 왜곡된다.
