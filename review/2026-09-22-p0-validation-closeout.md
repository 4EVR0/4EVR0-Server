# P0 검증 진행 결과 — 사람 교정 실패·멀티턴 동등성·결정론적 가드
...
- 사람 교정에 실패한 `resp_overall`·`grounding`·`format_adherence`는 통과 기준에서 제외하고,
...
## 6. 사람 블라인드 채점 40건 — 기존 Judge 불합격

- 라벨러: `hyeokjun-v7-rubric2`
- 고정 표본: `sample=40`, `seed=23`
- 비교 대상: 사람이 실제로 본 `v7-baseline-rubric2.json`의 동일 응답·동일 judge 점수
- 사람 평균: **4.120**, judge 평균: **4.720** — judge가 평균 **0.600점 과대평가**

| 차원 | MAE | Pearson | Spearman | judge−human 평균 |
|---|---:|---:|---:|---:|
| concern_fit | 0.775 | 0.3674 | 0.3570 | +0.775 |
| grounding | 0.575 | 0.3545 | 0.3924 | +0.475 |
| conciseness | 0.9667 | -0.0921 | -0.0465 | -0.367 |
| korean_quality | 1.0083 | 0.4049 | 0.3706 | +1.008 |
| format_adherence | 1.1083 | 0.0512 | 0.1120 | +1.108 |
| **전체** | **0.8867** | **0.0862** | **0.0468** | **+0.600** |

전체 상관이 사실상 0이고, 간결성·형식 준수는 사람의 순위를 거의 재현하지 못한다.
따라서 `gpt-4o-mini + response_judge(2e1ea732)` 점수를 현재 상태로 품질 게이트나
"4.7/5" 품질 주장에 사용할 수 없다. P0의 사람 교정 종료 조건은 **불합격**이다.
...
현재 P0는 **미완료**다. 사람 라벨 수집 자체는 완료했지만 기존 LLM Judge의 타당성이
기준에 못 미쳤고, 최신 코드의 라이브 멀티턴/단일턴 재실행도 외부 서버가 꺼져 있어 남았다.
