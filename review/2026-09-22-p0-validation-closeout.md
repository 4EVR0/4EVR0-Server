...
- `eval/hard_checks.py` 한 곳에서 단일 응답과 멀티턴 최종 결과를 검사한다.
- 실패 코드: 빈 응답, 한자, 미검증 제품 제약, 미등록 제품, 제품별 성분 근거 위반,
  제품 목적 불일치, 브랜드 중복, 소비자 금칙 표현.
- 런타임 가드 발동 자체는 실패가 아니다. 가드 적용 뒤 사용자에게 전달된 결과에
  위반이 남았을 때만 `hard_failures`로 기록한다.
- CI 생성 게이트는 개별 한자 지표 대신 모든 결정론적 위반을 포괄하는
  `hard_failure_rate == 0`을 요구하며, PR 표에 케이스별 코드와 상세를 노출한다.
